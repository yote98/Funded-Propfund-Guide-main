...
const ComparisonPage: React.FC<ComparisonPageProps> = ({ viewFirm, firms, initialSearchTerm, onSearchHandled }) => {
    ...
    // Removed duplicate Trading Categories filter (kept Category)
    ...
    const allCountries = useMemo(() => {
        const countries = firms.map(f => f.countryFounded).filter(Boolean);
        return Array.from(new Set(countries)).sort();
    }, [firms]);

    // Trading categories UI removed to avoid duplication with Category
    ...
    // Handler removed with trading categories UI
    ...
    const handleClearFilters = () => {
        ...
        // trading categories cleared (no longer used)
    ...
    const filteredAndSortedFirms = useMemo(() => {
        ...
        if (selectedCountries.length > 0) {
            filteredFirms = filteredFirms.filter(firm => 
                firm.countryFounded && selectedCountries.includes(firm.countryFounded)
            );
        }
        
        // trading categories filter removed (duplicate of Category)
    ...
    <div>
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm mb-8">
        <div className="space-y-4">
          ...
          {/* Country Filters */}
          <div>
            <h4 className="text-sm font-semibold text-slate-700 mb-3 text-center">Country</h4>
            <div className="flex flex-wrap justify-center gap-2">
          ...
          {/* Trading Categories section removed to avoid duplication with Category */}
                      ...
                      <tr key={firm.id} className="even:bg-gray-50 hover:bg-teal-50/50 transition-colors">
                        ...
                        <td className="px-4 py-4 whitespace-nowrap text-sm text-slate-600 font-medium">
                          <div className="flex items-center gap-1">
                            ...
                            {firm.trustScore && (
                              <span className="text-xs text-slate-500">({firm.trustScore})</span>
                            )}
                          </div>
                          {firm.trustedByUsers && (
                            <div className="text-xs text-slate-500 mt-1">
                              Trusted by {firm.trustedByUsers.toLocaleString()}
                            </div>
                          )}
                        </td>
                        <td className="px-4 py-4 whitespace-nowrap text-sm text-slate-600 font-medium">
                          <div>${firm.details.maxFunding.toLocaleString()}</div>
                          {firm.isDiscounted && firm.discountedPrice && (
                            <div className="text-xs text-green-600 font-bold">
                              ${firm.discountedPrice} {firm.discountPercentage && `(${firm.discountPercentage}% OFF)`}
                            </div>
                          )}
                        </td>
