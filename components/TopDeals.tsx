...
const TopDeals: React.FC<TopDealsProps> = ({ firms }) => {
  ...
  const PROMO_CODE = 'FPG10';

  const topDeals = useMemo(() => firms.filter(firm => firm.deal?.isTopDeal).slice(0, 5), [firms]);
          ...
          topDeals.map(firm => (
            <div key={firm.id} className="flex items-center justify-between p-3 border border-slate-200 rounded-lg bg-gray-50/50">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 bg-white rounded-md flex items-center justify-center p-1 border border-slate-200 flex-shrink-0">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-600">{firm.deal?.description}</p>
                </div>
              </div>
              <a 
